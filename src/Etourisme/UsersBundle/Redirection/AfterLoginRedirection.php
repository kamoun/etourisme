...
namespace Etourisme\UsersBundle\Redirection;
...
class AfterLoginRedirection implements AuthenticationSuccessHandlerInterface {
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token) {
        ...
        if (in_array('ROLE_ADMIN', $rolesTab, true) || in_array('ROLE_SUPER_ADMIN', $rolesTab, true))
            $redirection = new RedirectResponse($this->router->generate('users_homepage'));
        // sinon, s'il s'agit d'un commercial on le redirige vers le CRM
        elseif (in_array('ROLE_UTILSATEUR', $rolesTab, true))
            $redirection = new RedirectResponse($this->router->generate('users_homepage'));
